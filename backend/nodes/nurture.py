...
class NurtureEmailService:
    """Sends COLD lead nurture emails via SendGrid asynchronously."""

    def __init__(self):
        self.sg = sendgrid.SendGridAPIClient(api_key=settings.SENDGRID_API_KEY)

    async def send_cold_nurture(self, lead: LeadData, score: LeadScore):
        ...
        try:
            response = await anyio.to_thread.run_sync(lambda: self.sg.send(message))
            logger.info(
...
def get_nurture_service():
    global _nurture
    if _nurture is None:
        _nurture = NurtureEmailService()
    return _nurture

async def nurture_cold_lead(state: WorkflowState) -> WorkflowState:
    """Node 4c — Send COLD lead nurture email."""
    log = state.get("processing_log", [])
    nurture = get_nurture_service()
    try:
        await nurture.send_cold_nurture(state["lead"], state["score_result"])
        log.append("COLD nurture email sent")
    except Exception as e:
        log.append(f"COLD nurture FAILED: {e}")
    return {**state, "nurtured": True, "processing_log": log}